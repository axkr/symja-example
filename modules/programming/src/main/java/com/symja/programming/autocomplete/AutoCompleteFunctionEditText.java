...
public class AutoCompleteFunctionEditText extends androidx.appcompat.widget.AppCompatMultiAutoCompleteTextView {
    ...
    private static Pattern sFunctionPattern;

    private final Handler mHandler = new Handler();
    private final HighlightWatcher highlightWatcher = new HighlightWatcher();
    private boolean isEnableTextListener;
    private final Runnable mUpdateHighlight = () -> highlight(getEditableText());
    @Nullable
    ...
    private void init() {
        ...
        if (sFunctionPattern == null) {
            ...
            patternStr.append(")\\b");
            sFunctionPattern = Pattern.compile(patternStr.toString(),
    ...
    private void enableTextChangeListener() {
        if (!isEnableTextListener) {
            addTextChangedListener(highlightWatcher);
    ...
    private void disableTextChangeListener() {
        ...
        removeTextChangedListener(highlightWatcher);
    }
    ...
    public void highlight(Editable editable) {
        ...
        Matcher matcher = sFunctionPattern.matcher(s);
        while (matcher.find()) {
