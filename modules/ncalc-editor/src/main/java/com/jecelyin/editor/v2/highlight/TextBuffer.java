...
public class TextBuffer {
    ...
    public void getText(int start, int length, Segment seg) {
        if (start < 0 || length < 0
                || start + length > mEditable.length())
            throw new ArrayIndexOutOfBoundsException(start + "+" + length + " > " + mEditable.length());
    ...
    private void getTextByContentManager(int start, int len, Segment seg) {
        ...
        mEditable.getChars(start, start + len, dest, 0);
        seg.array = dest;
        seg.offset = 0;
        seg.count = len;
    }
    ...
    public void insert(int offset, CharSequence seq) {
        ...
        if (offset < 0 || offset > mEditable.length())
